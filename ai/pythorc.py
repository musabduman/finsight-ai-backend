...
import numpy as np
import pandas as pd
import joblib  # .gz ve .joblib dosyalarını açmak için
import os


class deeplearning:
    def __init__(self):
        # Dosya yollarını belirliyoruz (Streamlit/FastAPI'de hata almamak için os.path kullanmak iyidir)
        klasör = os.path.dirname(__file__)  # Şu anki dosyanın olduğu klasör
        model_yolu = os.path.join(klasör, "kahin_model.joblib")
        x_scaler_yolu = os.path.join(klasör, "x_scaler.gz")
        y_scaler_yolu = os.path.join(klasör, "y_scaler.gz")

        try:
            # RandomForest modelini yükle (artık state_dict değil, doğrudan model nesnesi)
            self.model = joblib.load(model_yolu)
    ...
    def analiz_et(self, df):
        ...
        # Sadece en son günün/saatin verisini tahmin için ayırıyoruz
        son_gün_verisi = df_clean[features].iloc[[-1]].values

        # Aynı x_scaler'ı kullanıyoruz (RandomForest scale'e ihtiyaç duymasa da
        # eğitimle inference arasında pipeline tutarlılığı için koruyoruz)
        son_gün_scaled = self.x_scaler.transform(son_gün_verisi)

        # 4. TAHMİN (INFERENCE) — artık torch.no_grad() yok, gerek de yok
        cikti = self.model.predict(son_gün_scaled)  # shape: (1,)

        # 0-1 arasındaki sonucu tekrar TL bazında fiyata çeviriyoruz
        tahmin_fiyat = self.y_scaler.inverse_transform(cikti.reshape(-1, 1))[0][0]
