# routers/analysis.py — Hisse analizi, AI chat, haber, hızlı tarama
#
# Eskiden ai_server.py adında bağımsız bir Render servisiydi.
...
# --- MODEL YÜKLEME ---
# Uygulama başlarken bir kere yükleniyor, tüm isteklerde paylaşılıyor.
# dl_bot içinde instance state tutuluyorsa eş zamanlı isteklerde
# sorun çıkabilir — şimdilik tek kullanıcılı/az trafikli proje için sorunsuz.
dl_bot = deeplearning()
...
@router.get("/api/quick-scan/{sembol}")
async def quick_scan(sembol: str):
    """Mega Tarama / BIST30 için hızlı teknik + DL tarama. Her hisse için ayrı çağrılır."""
    try:
